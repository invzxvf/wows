...
С переделкой инерционки пробитие крейсеров считается по-разному для уровней `[1, 8)` и `[8, 10]`, они вынесены в отдельные колонки. Помимо этого, пробитие фугасов немцев и британских тяжей (с калибром 234 мм) составляет 1/4 калибра, поэтому около значения пробития будет указана нация, если потребуется. Ниже таблица пробития для крейсерских калибров, начиная с 5-го тира.
